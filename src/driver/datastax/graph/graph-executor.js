...
const graphSON2Reader = new GraphSON2Reader({
	serializers: getCustomTypeSerializers(),
})
const graphSON2Writer = new GraphSON2Writer({
	serializers: getCustomTypeSerializers(),
})
const graphSON3Reader = new GraphSON3Reader({
	serializers: getCustomTypeSerializers(),
})
const graphSON3Writer = new GraphSON3Writer({
	serializers: getCustomTypeSerializers(),
})
...
class GraphExecutor {
	...
	constructor(client, rawOptions, handler) {
		...
		this._defaultProfileRetryPolicy =
			client.profileManager.getDefaultConfiguredRetryPolicy()
		...
		this._graphBaseOptions = utils.extend(
			...
			rawOptions.graphOptions,
			client.profileManager.getDefault().graphOptions,
		)

		if (this._graphBaseOptions.readTimeout === null) {
			this._graphBaseOptions.readTimeout =
				client.options.socketOptions.readTimeout
		}
	}
	...
	async send(query, parameters, options) {
		...
		parameters = GraphExecutor._buildGraphParameters(
			parameters,
			execOptions.getGraphSubProtocol(),
		)
		...
		if (typeof query !== "string") {
			...
			if (isQueryObject) {
				queryWriter = query.queryWriterFactory(
					execOptions.getGraphSubProtocol(),
				)
			} else if (!queryWriter) {
				queryWriter = GraphExecutor._writerFactory(
					execOptions.getGraphSubProtocol(),
				)
			}
	...
	async _executeGraphQuery(query, parameters, execOptions) {
		const result = await this._handler.call(
			this._client,
			query,
			parameters,
			execOptions,
		...
		const rowParser =
			execOptions.getRowParser() ||
			GraphExecutor._rowParserFactory(execOptions.getGraphSubProtocol())
	...
	async _getAnalyticsMaster() {
		try {
			const result = await this._client.execute(
				"CALL DseClientTool.getAnalyticsGraphServer()",
				utils.emptyArray,
			)
			...
			const hostName = resultField["location"].substr(
				0,
				resultField["location"].lastIndexOf(":"),
			)
			const addressTranslator =
				this._client.options.policies.addressResolution

			return await new Promise((resolve) => {
				addressTranslator.translate(
					hostName,
					this._client.options.protocolOptions.port,
					(endpoint) => resolve(this._client.hosts.get(endpoint)),
				)
			})
		} catch (err) {
			this._client.log(
	...
	_setGraphProtocol(execOptions) {
		...
		if (execOptions.getGraphName()) {
			const keyspace =
				this._client.metadata.keyspaces[execOptions.getGraphName()]
