...
function LineString(point) {
	...
	if (points.length === 1) {
		throw new TypeError(
			"LineString can be either empty or contain 2 or more points",
		)
	}
	/**
...
LineString.fromBuffer = function (buffer) {
	if (!buffer || buffer.length < 9) {
		throw new TypeError(
			"A linestring buffer should contain at least 9 bytes",
		)
	}
	const endianness = Geometry.getEndianness(buffer.readInt8(0, true))
	let offset = 1
	if (
		Geometry.readInt32(buffer, endianness, offset) !==
	...
	if (buffer.length !== offset + length * 16) {
		throw new TypeError(
			util.format(
				"Length of the buffer does not match %d !== %d",
				buffer.length,
				offset + length * 8,
			),
		)
