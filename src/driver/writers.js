...
class FrameWriter {
	...
	write(version, streamId, flags) {
		...
		const totalLength = headerBuffer.length + this.bodyLength

		// allocate a single buffer and copy all pieces to avoid Buffer.concat overhead
		const result = utils.allocBufferUnsafe(totalLength)
...
class WriteQueue extends events.EventEmitter {
	...
	process() {
		...
		while (
			this.queue.length > 0 &&
			totalLength < this.coalescingThreshold
		) {
			const writeItem = this.queue.shift()
			if (!writeItem.operation.canBeWritten()) {
				writeItem.callback(
					new Error(
						"The operation was already cancelled or timeout elapsed",
					),
				)
				continue
			}
			let data
			try {
				data = writeItem.operation.request.write(
					this.encoder,
					writeItem.operation.streamId,
				)
			} catch (err) {
		...
		// manually copy into a single buffer to avoid Buffer.concat overhead
		const combined = utils.allocBufferUnsafe(totalLength)
		let offset = 0
		...
		// Write combined buffer to the socket
		// Further writes will be throttled until flushed
		const self = this
		this.canWrite = self.netClient.write(combined, (err) => {
			if (err) {
				self.setWriteError(err)
				return
			}

			if (self.queue.length === 0 || !self.canWrite) {
