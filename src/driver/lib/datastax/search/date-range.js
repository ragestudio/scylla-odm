...
DateRange.prototype.toBuffer = function () {
	...
	if (
		this._type === dateRangeType.openBoth ||
		this._type === dateRangeType.openSingle
	) {
	...
	if (!matches) {
		return new DateRange(
			DateRangeBound.toLowerBound(
				DateRangeBound.fromString(dateRangeString),
			),
		)
	}
	return new DateRange(
...
DateRange.fromBuffer = function (buffer) {
	if (buffer.length === 0) {
		throw new TypeError(
			"DateRange serialized value must have at least 1 byte",
		)
	}
...
DateRangeBound.prototype.toString = function () {
	...
	for (i = 4; i < isoString.length && precision <= this.precision; i++) {
		...
		if (
			(precision >= dateRangePrecision.hour && char === ":") ||
			char === "."
		) {
			precision++
			continue
...
BoundaryBuilder.prototype.set = function (index, value, stringDate) {
	...
	switch (index) {
		...
		case dateRangePrecision.second:
			if (numValue > 59) {
				throw new TypeError(
					"Minute/second portion is not valid for date: " +
						stringDate,
				)
			}
			break
