...
class UdtSerializer {
	...
	serialize(udtWrapper) {
		...
		serializedValue["value"] = Object.entries(udtWrapper.value).map(
			([_, v]) => this.writer.adaptObject(v),
		)
...
const graphSONSerializerByCqlType = {
	...
	[dataTypes.timestamp]: new InternalSerializer("g:Timestamp", (x) =>
		x.getTime(),
	),
}
...
class GraphTypeWrapperSerializer {
	...
	serialize(wrappedValue) {
		...
		if (!s) {
			throw new Error(
				`No serializer found for wrapped value ${wrappedValue}`,
			)
		}
