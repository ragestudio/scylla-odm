...
class TypeSerializer {
	serialize() {
		throw new Error(
			"serialize() method not implemented for " + this.constructor.name,
		)
	}

	deserialize() {
		throw new Error(
			"deserialize() method not implemented for " + this.constructor.name,
		)
	}

	canBeUsedFor() {
		throw new Error(
			"canBeUsedFor() method not implemented for " +
...
class BytecodeSerializer extends TypeSerializer {
	...
	_serializeInstructions(instructions) {
		...
		for (let i = 0; i < instructions.length; i++) {
			result[i] = instructions[i].map((item) =>
				this.writer.adaptObject(item),
			)
		}
		return result
	}
...
class PSerializer extends TypeSerializer {
	...
	serialize(item) {
		...
		if (item.other === undefined || item.other === null) {
			...
			resultValue["value"] = [
				this.writer.adaptObject(item.value),
...
class TraverserSerializer extends TypeSerializer {
	...
	deserialize(obj) {
		...
		return new t.Traverser(
			this.reader.read(value["value"]),
			this.reader.read(value["bulk"]),
		)
	}
	...
	deserialize(obj) {
		...
		for (let ix = 0, iy = value.length; ix < iy; ix += 2) {
			...
			result = result.concat(
				Array(this.reader.read(pair[1])).fill(
					this.reader.read(pair[0]),
				),
			)
		}
...
class MapSerializer extends TypeSerializer {
	deserialize(obj) {
		...
		for (let i = 0; i < value.length; i += 2) {
			result.set(
				this.reader.read(value[i]),
				this.reader.read(value[i + 1]),
			)
		}
		return result
	}
